...
export async function controllerFixture(params: {
  ...
  uniV3Router: string,
  uniV3Factory: string
}, [wallet]: Wallet[], provider: MockProvider): Promise<IHotPotV3FundController> {
  const controllerFactory = await ethers.getContractFactory('HotPotV3FundController')
  return (await controllerFactory.deploy(params.hotpot, params.governance, params.uniV3Router, params.uniV3Factory, params.weth9)) as IHotPotV3FundController
}
...
export async function factoryFixture(params: {
  ...
  const factory = (await factoryFactory.deploy(params.controller, params.weth9, params.uniV3Factory, params.uniV3Router)) as IHotPotV3FundFactory
...
export interface CompleteFixture {
  ...
  tokenHotPot: IHotPot,
  factory: IHotPotV3FundFactory,
  controller: IHotPotV3FundController,
  positionLib: string,
  fundByteCode: string,
  quoter: IQuoter,
...
async function completeFixture(wallets: Wallet[],
  ...
  const fundByteCode = (await ethers.getContractFactory('HotPotV3Fund', {
    libraries: {
